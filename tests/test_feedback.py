...
def test_submit_requires_sentiment_and_ack(home):
    assert "traffic light" in feedback.submit("", "", "", "", True)["error"]
    assert "required" in feedback.submit("green", "", "", "", False)["error"]
...
def test_submit_assembles_and_posts(home):
    ...
    assert p["name"] == ""            # placeholder until onboarding collects it
