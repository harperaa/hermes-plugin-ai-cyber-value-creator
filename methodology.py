...
def build_task_description(
    ...
    foundation_brief = [
        ...
        "   'Q3 (company size)'. Do NOT post cards or block with raw kanban tools —",
        "   only fall back to manual kanban comment (marker line `### ❓ QUESTION",
        "   FOR YOU`, then `**Question <n> of <total>**`) + block(kind=needs_input,",
        "   unique reason) if `ask_user_question` is unavailable.",
        ...
        "3. **If you are NOT in a live chat** (fresh worker run), END YOUR TURN",
        "   after `ask_user_question` by restating the complete question — options",
        "   included — as your final chat message, noting they can answer here or",
        "   on either card. Never end with just a pointer to a card.",
        ...
        "   question. If their answer arrived as a chat reply instead, call",
        "   `record_user_answer` with it (keeps all three surfaces in sync and",
        "   resets the board's stuck-loop counter; the task deliberately stays",
        "   blocked while you keep working it here).",
