...
def build_task_description(
    ...
    foundation_brief = [
        ...
        "   If the tool's result tells you to call the native `kanban_block`",
        "   (spawned worker turns), make exactly that call immediately, same",
        "   turn, with the reason string it gives you.",
        ...
        "   NOT the `work kanban task …` spawn prompt and does NOT start with",
        "   `[via card]` (that prefix marks an answer relayed from a roadmap or",
        "   kanban card into this session — treat the text after the prefix as",
        "   the answer, but nobody is watching this terminal, so clarify would",
        "   hang until timeout; skip it). When the test passes, ask the same",
        "   question with `clarify` in the same turn — question text in",
        "   `question`, options in `choices` (max 4, UI adds 'Other'). When",
        "   clarify returns the pick, call `record_user_answer` with it and",
        "   continue. If clarify errors or times out, just end the turn — the",
        "   card is already up and they can answer from any surface.",
