...
def build_task_description(
    ...
    foundation_brief = [
        ...
        "card, or by replying in this chat thread — and every question must be",
        "answerable from all three. To ask:",
        "1. Post ONE kanban comment whose FIRST line is exactly:",
        "   `### ❓ QUESTION FOR YOU`",
        "   and whose SECOND line states the interview progress exactly as",
        "   `**Question <n> of <total>**` — n = this question's number in this",
        "   step's interview, total = the full number of questions you plan to",
        "   ask for this step (count them from the guidance skill BEFORE asking",
        "   the first one; keep the total stable unless the plan genuinely",
        "   changes). The roadmap card reads this line and draws a progress bar",
        "   so the user can see how close the topic is to done.",
        "   The comment contains EXACTLY ONE question. When the answer is a choice between",
        "   discrete options, list each option on its own line starting with '- '.",
        "   The dashboards turn EVERY '- ' (or 'A)'-style) line in the card into a",
        "   clickable answer button — so option lines are the ONLY lines allowed",
        "   to start that way. Progress recaps ('locked in so far' etc.) do NOT",
        "   belong in the question card: post them as a separate normal comment",
        "   (no marker) or plain prose, never as dash bullets inside the card.",
        ...
        "3. END YOUR TURN by restating the COMPLETE question — options included —",
        "   as your final chat message, and note the user can answer right here",
        "   in chat or on either card. The user may only be reading this chat",
        "   thread: never end with just a pointer to a card.",
        "4. When you resume, the user's answer is in the newest comments — read",
        "   them FIRST, acknowledge, and continue. Never re-post an answered",
        "   question.",
        "5. **Be FAST between questions — the user is waiting live.** When you",
        "   spawn/resume and the answer is already in the comments you were just",
        "   shown, do EVERYTHING in ONE assistant turn: the (optional, short)",
        "   lock-in note comment, the next question-card comment, and the block",
        "   call — all as tool calls of that single turn. Do NOT re-read the",
        "   company-context file, run tool searches, or maintain a todo list",
        "   between questions; the task body + comments already contain",
        "   everything you need. Record to the company context only when a",
        "   field is fully confirmed (typically once, at the end).",
        "",
        "**If the user answers YOU here in the chat thread** (this conversation",
        "resumes with their reply instead of a card answer): call the",
        "`record_user_answer` tool with this kanban task's id and their exact",
        "answer — it posts the answer onto the task thread (keeping all three",
        "surfaces in sync) and resets the board's stuck-loop counter, while the",
        "task deliberately STAYS blocked because you are already here working it.",
        "Then continue the interview in this same conversation: post the next",
        "question-card comment as usual but do NOT call the block tool again (the",
        "task is already blocked), and restate each new question in chat. When",
        "the interview finishes, record the result, then unblock and complete",
        "the task as normal.",
        "",
