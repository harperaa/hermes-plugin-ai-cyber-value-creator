...
def build_task_description(
    ...
    foundation_brief = [
        ...
        "",
        "**HOW TO ASK THE USER (chat-only interview — MANDATORY).** THIS chat",
        "session is the ONLY place questions are asked and answered. The roadmap",
        "and kanban boards show status only (working / waiting / progress bar) —",
        "NEVER post questions, options, recaps, or answers as kanban comments.",
        "Plan the interview BEFORE the first question: count the questions from",
        "the guidance skill and keep that total stable unless the plan genuinely",
        "changes. Then, for EVERY question:",
        "1. Call the `ask_user_question` tool — it records the interview position",
        "   (the user's progress bar) and marks the task 'waiting for your",
        "   answer'. Parameters, exactly: task_id, question_number,",
        "   question_total, reason_tag (short unique tag like 'company size').",
        "   Call it DIRECTLY — never spend calls on tool_describe/tool_search;",
        "   the schema is fully stated right here. If its result tells you to",
        "   call the native `kanban_block` (spawned worker turns), make exactly",
        "   that call immediately, same turn, with the reason string it gives.",
        "2. **clarify — ONLY when the user is truly live in this chat.** The",
        "   deterministic test: the latest user message was TYPED here — it is",
        "   NOT the `work kanban task …` spawn prompt. When the test passes, ask",
        "   the question with `clarify` in the same turn — question text in",
        "   `question`, options in `choices` (max 4, the UI adds 'Other'). When",
        "   clarify returns the pick, acknowledge it and continue. If clarify",
        "   errors or times out, just end the turn per step 3.",
        "3. **ALWAYS end the turn with the question itself.** Your FINAL chat",
        "   message MUST contain the complete question — options included as a",
        "   NUMBERED list (1., 2., …) — and close with one short line telling",
        "   the user how to continue, e.g. 'Reply with a number or type your",
        "   answer.' A bare option number typed in chat is a valid answer —",
        "   never ask them to restate it. This applies in every session type:",
        "   the chat thread is the only place the user can read and answer it,",
        "   and a final message that merely says a question was asked (or names",
        "   its number) is a protocol violation.",
        "4. **Answers arrive as the next incoming chat message.** Acknowledge,",
        "   then continue with the next question (steps 1-3) in ONE assistant",
        "   turn — no todo lists, no tool searches, no re-reading the",
        "   company-context file. The task deliberately STAYS blocked for the",
        "   whole interview — never unblock between questions. Never re-ask an",
        "   answered question — re-read this conversation first.",
        "5. **Finish cleanly.** When the interview completes: record the result",
        "   via `record_company_context` (typically once, at the end), then",
        "   unblock and complete the task as normal.",
        "",
